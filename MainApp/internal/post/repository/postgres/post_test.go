...
func TestRepositoryCreatePost(t *testing.T) {
	...
	mockPost.Images = make([]models.Image, 2)

	mockPost.Images[0] = models.Image {
		ID: 1,
		ImgLink: "link1",
	}

	mockPost.Images[1] = models.Image {
		ID: 2,
		ImgLink: "link2",
	}
    ...
    `INSERT INTO "user_posts_images" ("user_post_id","img_id") VALUES ($1,$2),($3,$4)`)).
	WithArgs(mockPost.ID, mockPost.Images[0].ID, mockPost.ID, mockPost.Images[1].ID).
...
func TestRepositoryUpdatePost(t *testing.T) {
    ...
    mock.ExpectExec(regexp.QuoteMeta(
    `DELETE FROM "user_posts_images" WHERE "user_posts_images"."user_post_id" = $1`)).
